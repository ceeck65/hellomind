...
},{"stupid-callctrl":7,"stupid-changed":9,"stupid-event":8,"stupid-iterator":10}],12:[function(require,module,exports){
/**
...
},{"stupid-event":8}],14:[function(require,module,exports){
...
},{"stupid-deferred":13,"stupid-imageloader":15}],15:[function(require,module,exports){
var Deferred = require('stupid-deferred');
...
},{"stupid-deferred":13}],16:[function(require,module,exports){
...
},{"stupid-deferred":13,"stupid-event":8,"stupid-imagesloader":14}],17:[function(require,module,exports){
...
/** @export */
module.exports = Tick;
},{"stupid-callctrl":17}],19:[function(require,module,exports){
/*
	Dependencies
*/
...
function AudioConstructor(options) {
	...
	*/
	...
	var player = Player({el: el.querySelector('.partial-player')});
	...
	*/
	...
	function progress() {
		...
		var _progress = audio.currentTime / audio.duration;
		
		player.set(_progress);	
	}
...
module.exports = AudioConstructor;
},{"./player":43}],20:[function(require,module,exports){
/*
	Dependencies
*/

function BackgroundImage(options) {
...
var down = '(max-width: 1023px)';
var up = '(min-width: 1024px)';

var isDown;
var isUp;

window.addEventListener('resize', matchBreakpoints);

function matchBreakpoints() {
	isDown = window.matchMedia(down).matches ? true : false;
	isUp = window.matchMedia(up).matches ? true : false;

	var triggerEvent = self.isDown != isDown ? true : false;

	self.isDown = isDown;
	self.isUp = isUp;

	//log.log('breakpoints', 'breakpoints', 'Down', isDown, 'Up', isUp);
...
module.exports = self;
},{"./log":34,"stupid-event":8}],22:[function(require,module,exports){
/*
	Dependencies
*/

var Event = require('stupid-event');
var Background = require('./parallax/background');
var PosterImage = require('./parallax/poster-image');
var Text = require('./parallax/text');
var TriggerElement = require('./parallax/trigger-element');
var Sprite = require('./deck/sprite');
...
function Deck(options) {
	...
	*/
	...
	var el = options.el;
	var scrollspy = options.scrollspy;
	var deck;
	...
	var sprite = el.querySelector('.partial-sprite');
	sprite = sprite ? Sprite({el: sprite, scrollspy: scrollspy, bindEvents: false}) : undefined;

	var elements = {};
	elements.background = el.querySelector('.background');
	if(elements.background) {
		var background = Background({el: elements.background});

		background.on('complete', showText);
	}

	elements.triggerElement = el.querySelector('.trigger-element');
	if(elements.triggerElement) {
		var triggerElement = TriggerElement({
			el: elements.triggerElement,
			stop: content
		});
	}

	elements.posterImages = [].slice.call(el.querySelectorAll('.component-poster-image'));

	if(elements.posterImages) {
		var posterImages = [];
		elements.posterImages.forEach(function(_el) {
			var posterImage = PosterImage({el: _el});

			posterImages.push(posterImage);
		});
		
	}

	var text = Text({elements: [].slice.call(el.querySelectorAll('.inner'))});
	...
	*/
	...
	function bindEvents() {
		if(deck) return;
		deck = scrollspy.add(el);

 		deck.on('active', active);
 		deck.on('hidden', hidden);
		deck.on('deactive', deactive); 
		deck.on('progress', progress); 

		/*
			Set {log}
		*/
		//log.log('deck', 'deck', 'Events binded');
	}

	function unbindEvents() {
		if(deck) {
			scrollspy.remove(deck);
			deck = null;
			//log.log('deck', 'deck', 'Events unbinded');
		}
	}
	...
	*/
	...
	function active(_el, _direction) {
		if(self.disabled) return;

		//log.log('deck' + id, 'deck', 'Active');

		var direction = _direction == 'bottom' ? 1 : -1;
		//text.show(direction);

		/*
			Trigger {active} event
		*/
		event.trigger('active', _el, _direction);	
	}

	function deactive(_el, _direction) {
		if(self.disabled) return;

		//log.log('deck' + id, 'deck', 'Deactive');

		/*
			Trigger {deactive} event
		*/
		event.trigger('deactive', _el, _direction);	
	}

	function hidden(_el, _direction) {
		if(self.disabled) return;

		//log.log('deck' + id, 'deck', 'Hidden');

		var direction = _direction == 'top' ? 1 : -1;
	}

	function progress(_el, _direction, _progress) {
		if(self.disabled) return;

		//log.log('deck' + id, 'deck', 'Progress', Math.round(_progress * 100));
		/*
			Transform direction {Int}
		*/
		var direction = _direction == 'bottom' ? 1 : -1;
		...
		/*
			Move background if present
		*/
		if(background) {
			background.move(_progress);

			/*
				Check whether text is within the background's boundary
			*/
			if(triggerElement) {
				if(background.top <= triggerElement.top && background.bottom <= triggerElement.bottom) {
					showText();
				} else {
					text.hide(direction);
				}
	...
	function showText() {
		...
		if(background.easeTop <= triggerElement.top && background.easeBottom <= triggerElement.bottom) {
			//log.log('deck' + id, 'deck', 'Show text');
			text.show(self.direction);
		}
	}
	...
	function enable() {
		...
		if(triggerElement) 	triggerElement.refresh();
		if(text)			text.enable();
	...
	function disable() {
		...
		if(text)			text.disable();
		if(background) 		background.disable();
		
		/*
			Set {log}
		*/
		//log.log('deck', 'deck', 'Disable');
	}

	function refresh() {
		if(background) 		background.refresh();
		if(triggerElement) 	triggerElement.refresh();

		/*
			Set {log}
		*/
		//log.log('deck', 'deck', 'Refresh');
	}
...
module.exports = Deck;
},{"./deck/sprite":24,"./log":34,"./parallax/background":39,"./parallax/poster-image":40,"./parallax/text":41,"./parallax/trigger-element":42,"gsap":2,"stupid-event":8}],23:[function(require,module,exports){
/*
	Dependencies
*/
...
function Deck(Page) {
	var el = document.querySelector('#deck4');
	var videos = Page.videos;
	
	var gallery = Slider({el: el.querySelector('.partial-slider.gallery'), calculateHeight: false});
	var quotes = Slider({el: el.querySelector('.partial-slider.quotes')});
	var thumbnails = Thumbnails({el: document.querySelector('.partial-thumbnails')});

	thumbnails.on('click', function(index) {
		gallery.goToPage(index)
		quotes.goToPage(index)
	});

	gallery.on('scrollend', thumbnails.set)

	gallery.on('scroll', function(_progress) {
		quotes.scroll(_progress);
	});

	quotes.on('scroll', function(_progress) {
		gallery.scroll(_progress);	
	})



	videos.forEach(function(el) {
		el.on('beforetransition', gallery.disable);
		el.on('completed', gallery.enable);
	});
}

module.exports = Deck;
},{"../slider":46,"../thumbnails":49}],24:[function(require,module,exports){
/*
	Dependencies
*/

var SpriteConstructor = require('stupid-sprite');
var SmallLoader = require('../small-loader');
var Event = require('stupid-event');
...
function Sprite(options) {
	...
	*/
	...
	var images = JSON.parse(el.dataset.spriteImages);

	var loop = typeof el.dataset.spriteLoop === 'undefined' ? true : el.dataset.spriteLoop;
	var frameHeight = el.dataset.spriteFrameHeight || undefined;


	/*
		Sprite has low res sprite sheets
	*/
	if(breakpoints.isDown && el.dataset.spriteImagesDown) {
		images = JSON.parse(el.dataset.spriteImagesDown);
		frameHeight = el.dataset.spriteFrameHeightDown;
	}

	var startDelay = el.dataset.spriteStartDelay || false;
	var endDelay = el.dataset.spriteEndDelay || false;
	var timeout;
	var fallbackImage = el.dataset.spriteFallbackImage;
	var doBindEvents = options.bindEvents;
	var smallLoader = SmallLoader({el: el.querySelector('.partial-small-loader')});

	/*
		Disable sprite on breakpoint down
	*/
	if(breakpoints.isDown && el.dataset.spriteDisabledOnDown) {
	...
	*/
	function bindEvents() {
		...
		//log.log('sprite', 'sprite', 'Events binded');
		var scrollspyElement = scrollspy.add(el);

		scrollspyElement.on('visible', play);
		scrollspyElement.on('hidden', pause);
	}
	...
	*/
	...
	function success() {
		//log.log('sprite', 'sprite', 'Loaded');
		smallLoader.hide({
			onComplete: function() {
				if(doBindEvents) bindEvents();
			}
		});
	...
	function play() {
		//log.log('sprite', 'sprite', 'Play');

		if(startDelay) {
			sprite.play(0);
			sprite.stop(0);
			timeout = setTimeout(sprite.play, startDelay);
		} else {
			sprite.play();
		}
	}
	...
	function pause() {
		...
		//log.log('sprite', 'sprite', 'Pause');
		var frame = loop ? undefined : 0;
	...
	function onEnded() {
		if(loop) {
			if(endDelay) {
				timeout = setTimeout(ended, endDelay);
			} else {
				play();
			}
		}
	...
	function ended() {
		gsap.to(canvas, .5, {
			...
			onComplete: function() {
				sprite.stop(0);
				gsap.to(canvas, .5, {opacity: 1, onComplete: play });
			}
		})
	}
...
module.exports = Sprite;
},{"../breakpoints":21,"../log":34,"../small-loader":47,"./tick":25,"gsap":2,"stupid-event":8,"stupid-scrollspy":11,"stupid-sprite":16}],25:[function(require,module,exports){
var Tick = require('stupid-tick');
var tick = Tick({fps:25});

module.exports = tick;
},{"stupid-tick":18}],26:[function(require,module,exports){
/*
	Dependencies
*/
...
var breakpoints = require('./breakpoints');
var logo = require('./logo').getInstance();
...
function Decks(options) {
	...
	*/
	...
	var el = document.querySelector('.sections');
	var elements = options.elements;
	var scrollspy = options.scrollspy;
	var event = Event();
	//var marquee = Marquee({el: document.querySelector('.partial-marquee')})

	var list = List({el: document.querySelector('#frontpage-list')});

	var indicators = Indicators({el: document.querySelector('#frontpage-indicators')});
	...
	elements.forEach(function(el, i) {
		...
		var deck = Deck(_options);

		var indicator = indicators.add();

		deck.indicator = indicator;

		if(breakpoints.isDown) {
			deck.disable();
		}

		deck.on('active', function(_el, _direction) {
			var direction = getDirection(_direction);
			
			if(i == decks.length - 1 && direction == -1) {
				return;
			}

			list.set(i, direction);
			indicators.set(i, direction);

			// Reverse logo or not
			var _reverse = _direction == 'top' ? true : false;
			logo.play(_reverse);
		})

		deck.on('deactive', function(_el, _direction) {
			if(_direction == 'top' && i == decks.length - 1) return;
			//indicator.disable();
		})
	...
	window.addEventListener('resize', function() {
		decks.forEach(function(deck) {
			if(breakpoints.isUp) deck.refresh();
		})
	});
	...
	*/
	...
	function render() {
		...
		decks.forEach(function(deck) {
			...
			if(breakpoints.isUp) deck.enable();
		})
	}



	function getDirection(_direction) {
		return _direction == 'bottom' ? 1 : -1;
	}
...
},{"./breakpoints":21,"./deck":22,"./indicators":31,"./list":32,"./logo":35,"stupid-event":8}],27:[function(require,module,exports){
/*
	Dependencies
*/
var gsap = require('gsap');

var PageConstructor = require('./page');
var Decks = require('./decks');
var Sections = [require('./deck/deck4.js')];
...
function Page(options) {
	...
	*/
	...
	var self = PageConstructor(options);
	...
	var decks = Decks({
		elements: [].slice.call(document.querySelectorAll('#frontpage .sections .section')),
		scrollspy: self.scrollspy
	});

	loader.on('rendercomplete', function() {
		decks.render();
	});
	...
	*/
	...
	var readMore = document.querySelector('#frontpage .partial-list li:first-child');
	var deck2 = document.querySelector('#frontpage #deck2');

	readMore.addEventListener('click', function() {

		var bounds = deck2.getBoundingClientRect();

		var scrollY = window.scrollY;
		var tween = {y: scrollY};

		gsap.to(tween, 1.75, {y: bounds.top + scrollY, ease:Power3.easeOut, onUpdate: function() {
...
module.exports = Page;
},{"./deck/deck4.js":23,"./decks":26,"./page":38,"gsap":2}],28:[function(require,module,exports){
/*
	Dependencies
*/
...
function Hamburger(el) {
	...
	*/
	...
	var canvas = el.querySelector('.canvas');
	var images = [
		'/wp-content/themes/hellomind/static/images/sprite-hamburger.png'
	];
	...
	var toggle = Callctrl.shift(_open, _close);
	...
	*/
	...
	function onUpdate(_frame) {
		if(_frame === 3 && !isClicked) sprite.pause();
	}
...
},{"stupid-callctrl":7,"stupid-sprite":16,"stupid-tick":18}],29:[function(require,module,exports){
/*
	Polyfills
*/

var objectFitPolyfill = require('object-fit-polyfill');
...
switch(type) {
	case 'frontpage':
		...
		page = Frontpage({forceLoader: true});
		break;
	case 'page':
		var Page = require('./page');
		page = Page({forceLoader: false});
		break;
	case 'otherminds':
...
},{"./frontpage":27,"./navigation":37,"./page":38,"./shift":44,"object-fit-polyfill":5}],30:[function(require,module,exports){
/*
	Dependencies
*/
...
},{"gsap":2,"stupid-event":8}],31:[function(require,module,exports){
/*
	Dependencies
*/
...
function Indicators(options) {
	...
	*/
	...
	function add() {
		...
		el.appendChild( indicator.getElement() );

		return indicator;
	}

	function set(_index) {
		indicators[active].disable();
		active = _index;
		indicators[active].enable();
	}
...
},{"./indicator":30}],32:[function(require,module,exports){
/*
	Dependencies
*/
...
function List(options) {
	...
	*/
	...
	var ease = options.ease || Power2.easeOut;//Expo.easeOut;
	var offset = options.offset || 100;

	var active = 0;

	var elements = [].slice.call(el.querySelectorAll('li'));
	...
	var Element = {
		...
		enable: function(_direction) {
			var x = offset * _direction * -1;

			gsap.set(this.el, {x: x})
			gsap.to(this.el, duration, {x: 0, opacity: 1, ease: ease, force3D: true});
		},
		disable: function(_direction, _duration) {
			_duration = typeof _duration === 'undefined' ? duration : _duration;

			var x = offset * _direction;

			gsap.to(this.el, _duration, {x: x, opacity: 0, ease: ease, force3D: true});
		}
	}
	...
	*/
	...
	function set(_index, _direction) {
		//if(_index == active) return;
		elements[active].disable(_direction);
		active = _index;
		elements[active].enable(_direction);
...
module.exports = List;
},{"gsap":2,"stupid-event":8}],33:[function(require,module,exports){
/*
	Dependencies
*/
...
var Sprite = require('stupid-sprite');
var Tick = require('stupid-tick');
var Promise = typeof Promise !== "undefined" ? Promise : require('promise-polyfill');
...
function Loader(options) {
	...
	*/
	var self = {};
	var body = options.el;
	var el = body.querySelector('.component-loader');
	if(!el) return;
	
	// Create tick 
	var tick = Tick({fps:48});

	var spriteElem = el.querySelector('.sprite');
	var canvas = spriteElem.querySelector('canvas');
	var img = spriteElem.querySelector('img.init');
	var images = [spriteElem.dataset.spriteImage];

	// Whether to force animation to finish
	var forceLoader = options.forceLoader || false;
	...
	*/
	...
	/*
		Itterate through each loaders
	*/
	loaders.forEach(function(loader) {
		loader.init()
			.then(function() {
				//log.log('loader', 'loader', loader.name, 'loaded');
				loader.hasLoaded = true;
				checkGlobalLoaderStatus();
			})
			.catch(function() {
				log.log('loader', 'loader', loader.name, 'not loaded');
				loader.failed = true;
	...
	*/
	...
	/*
		Check global loader status
	*/
	function checkGlobalLoaderStatus() {
		/*
			Loop through all loaders to see their loading status
		*/
		var failed = false;

		for (var i = 0; i < loaders.length; i++) {
			if(!loaders[i].hasLoaded && !loaders[i].failed) {
				//log.log('loader', 'loader', 'Loading not complete');
				return;
			}
			if(loaders[i].failed) {
				failed = true;
		...
		/*
			Every loader has been loaded
		*/
		if(!failed) {
			//log.log('loader', 'loader', 'Loading complete. Every loader has been loaded');
		} else {
			//log.log('loader', 'loader', 'Loading complete. Some content has not been loaded');
		}
	...
	function complete() {
		...
		if(!forceLoader) {
			...
			sprite.on('ended', renderComplete);
		}
	}
...
},{"./log":34,"gsap":2,"imagesloaded":3,"promise-polyfill":6,"stupid-event":8,"stupid-sprite":16,"stupid-tick":18}],34:[function(require,module,exports){
var Log = {
    logIds: [],
    logItems: [],
    colors: ['maroon','green','teal','navy','olive','red','blue','purple'],
    active: true,
    getColor: function(){
        var colorIndex = (this.logIds.length - 1) % this.colors.length;
        return this.colors[colorIndex];
    },
    log: function(){
        var args = Array.prototype.slice.call(arguments);
        var id = args.shift();
        this.createLogItem(id);
        this.logItems[id].log(args);
    },
    history: function(_id){
        if(this.logItems[_id]) this.logItems[_id].showHistory();
    },
    disable: function(_id){
        if(_id){
            this.createLogItem(_id);
            if(this.logItems[_id]) this.logItems[_id].disable();    
        }else{
            this.active = false;
        }
    },
    enable: function(_id){
        if(_id){
            this.createLogItem(_id);
            if(this.logItems[_id]) this.logItems[_id].enable();    
        }else{
            this.active = true;
            for(key in this.logItems){
                this.logItems[key].enable();
            }
        }
    },
    createLogItem: function(_id){
        var index = this.logIds.indexOf(_id);
        if(index === -1){
            this.logIds.push(_id);
            this.logItems[_id] = LogItem.call(this, {
                id: _id
            });
        }
    }
    ...
    */
    ...
    function log(_args){
        ...
        if(typeof args[0] === 'string'){
            args[0] = '%c ' + args[0] + ' ';
            args.splice(1, 0, 'background:'+color+'; color: white;');
        }
        ...
        /** Log */
        if(!parent.active || !active) return;
    ...
    function showHistory(){
        if(!parent.active || !active) return;
        console.log('%c History of \'' + id + '\' ', 'background-color:black;color:white');
        for (var i = 0; i < history.length; i++) {
            console.log.apply(console, history[i]);
        }
    }

    function addHistory(_args){
        history.unshift(_args);
...
function Logo() {
	...
	*/
	...
	var images = [
		'/wp-content/themes/hellomind/static/images/sprite-logo.png'
	];
...
var Module = Singleton(Logo); 
module.exports = Module;
},{"stupid-singleton":12,"stupid-sprite":16,"stupid-tick":18}],36:[function(require,module,exports){
/*
	Dependencies
*/
...
function Marquee(options) {
	...
	*/
	...
	var framer = el.querySelector('.framer');
	var text = el.querySelector('.text');
	var span = text.querySelector('span');

	//text.appendChild(span.cloneNode(true));

	//text.style.paddingLeft = text.offsetWidth + 'px';


	function set(progress) {

		var x = progress * framer.offsetWidth * -1; 
...
},{"gsap":2}],37:[function(require,module,exports){
/*
	Dependencies
*/
...
function Navigaton() {
	...
	*/
	...
	var topbar = document.querySelector('.aside');

	var hamburger = Hamburger(trigger);
	var toggle = Callctrl.shift(show, hide);

	var languageSelector = el.querySelector('.topbar');

	var staggers = [
		Stagger({
			elements: [].slice.call(el.querySelectorAll('.nav--primary li, .nav--secondary'))
		}),
		Stagger({
			elements: [].slice.call(el.querySelectorAll('.column-secondary'))
		})
	];
	...
	*/
	...
	function setElements() {
		...
		background = document.querySelector('.navigation-background');
		trigger = document.querySelector('.aside .hamburger');
	}
	...
	function onClick() {
		isVisible ? toggle.beta() : toggle.alpha();
	}
	...
	function show() {
		...
		staggers.forEach(function(stagger) {
			stagger.show();
		})

		setHeight();
		gsap.to(el, duration, {height: height});

		gsap.to(background, duration, {scaleY: 1, onComplete: function() {
			//el.style.height = "";
			isVisible = true;
		}});
	...
	function resize() {
		setHeight();
		gsap.set(el, {height: height});
	}

	function setHeight() {
		height = breakpoints.isUp ? window.innerHeight : window.innerHeight - topbar.offsetHeight;	
	}
	...
	function hide() {
		...
		gsap.to(el, duration, {height: 0});

		gsap.to(background, duration, {scaleY: 0, onComplete: function() {
			el.classList.remove('active');
			body.style.overflow = "";
...
module.exports = Navigaton;
},{"./breakpoints":21,"./hamburger":28,"./stagger":48,"gsap":2,"stupid-callctrl":7}],38:[function(require,module,exports){
var logo = require('./logo').getInstance();
...
var BackgroundImage = require('./background-image');
...
function Page(options) {
	...
	var loader = Loader({
		el: document.body,
		forceLoader: options.forceLoader || false
	});
	var event = Event();

	var marquee = document.querySelector('.partial-marquee');

	if(marquee) {
		marquee = Marquee({el: marquee});
	}

	if(!options.disableSprites) {
		var spriteElements = [].slice.call(document.querySelectorAll('.partial-sprite'));

		var sprites = [];

		spriteElements.forEach(function(el) {
			var sprite = Sprite({ el: el});
			
			if(sprite) sprite.bindEvents();
	...
	function scroll() {
		var progress = window.pageYOffset / (document.documentElement.scrollHeight - document.documentElement.clientHeight);
		marquee.set(progress);
	}


	var imageElements = [].slice.call(document.querySelectorAll('.component-background-image'));
	var images = [];
	imageElements.forEach(function(el) {
		var image = BackgroundImage({el: el});
	...
	// Test video
	var videoElements = [].slice.call(document.querySelectorAll('.partial-video'));

	var videos = [];

	videoElements.forEach(function(el, i) {
		var video = Video({el:el});
		videos.push(video);
	})

	// Audio elements
	var audioElements = [].slice.call(document.querySelectorAll('.partial-audio'));

	var audios = [];

	audioElements.forEach(function(el) {
		var audio = Audio({el:el});
		audios.push(audio);
	})
...
module.exports = Page;
},{"./audio":19,"./background-image":20,"./breakpoints":21,"./deck/sprite":24,"./loader":33,"./logo":35,"./marquee":36,"./video":51,"promise-polyfill":6,"stupid-event":8,"stupid-scrollspy":11,"stupid-tick":18}],39:[function(require,module,exports){
/*
	Dependencies
*/
...
function Background(options) {
	...
	*/
	...
	var originY = el.dataset.backgroundOriginY || .5;
	var scaleY = el.dataset.backgroundScaleY || 0;// 0.2;
	var ease = options.ease || Power4.easeOut;
	var start = el.dataset.backgroundStart || 0;
	var end = el.dataset.backgroundEnd || 0.5;
	...
	gsap.set(el, {transformOrigin: '50% '+ originY * 100 +'%'});
	...
	*/
	...
	function enable() {
		...
		var sy = scaleY == 1 ? 0 : scaleY;

		set(sy);
	}

	function move(progress) {
		mappedScaleY = map(progress, start, end, scaleY, 1);

		self.top =  calculateTop(mappedScaleY);
		self.bottom = calculateBottom(mappedScaleY);

		gsap.to(tween, duration, {
			scaleY: mappedScaleY,
			ease: ease,
			onUpdate: draw,
			onComplete: function() {
	...
	function set(sy) {
		gsap.set(el, {scaleY: sy, force3D: true});
	...
	function disable() {
		self.disabled = true;
		//log.log('background', 'background', 'Disabled');
	...
	function draw() {
		if(self.disabled) return;
		//log.log('background', 'background', 'Draw');

		gsap.set(el, {scaleY: tween.scaleY});

		self.easeTop =  calculateTop(tween.scaleY);
		self.easeBottom = calculateBottom(tween.scaleY);

		event.trigger('draw');
	}

	function calculateTop(scale) {
		return (0 + originY) * (parentHeight - scale * height)
	}

	function calculateBottom(scale) {
		return (1 - originY) * (parentHeight - scale * height)
	}
	...
	*/
	...
	self.logEnabled = el.dataset.backgroundEnableLog;
	self.easeTop = 0;
	self.easeBottom = 0;
	self.on = event.on;
	self.top = 0;
...
},{"../log":34,"../utils/map":50,"gsap":2,"stupid-event":8}],40:[function(require,module,exports){
/*
	Dependencies
*/
...
function PosterImage(options) {
	...
	*/
	...
	var image = el.querySelector('.component-background-image');

	var ease = options.ease || Back.easeOut.config(1.7);
	ease = Power4.easeOut;
	var duration = options.duration || 2;


	var y = options.y || el.offsetHeight / 4;
	var x = options.x || 0;

	var scale = options.scale || 1.5;

	gsap.set(image, {y: y, x: x, top: -y, right: x});

	function set(progress) {
		gsap.to(image, duration, {
			ease: ease,
			y: y - (y * map(progress, 0, 1, 0, .5)),
			scale: map(progress, 0, .4, scale, 1)
		});
	}
	/*
		Public methods
...
},{"../utils/map":50,"gsap":2}],41:[function(require,module,exports){
/*
	Dependencies
*/
...
function Text(options) {
	...
	*/
	...
	var y = 0;// 0.2;
	var ease = options.ease || Power4.easeOut;
	var duration = options.duration || .45;
	var alwaysShow = elements[0].dataset.textAlwaysShow;
	var staggerDelay = .1;
	...
	*/
	...
	function show(direction, delay) {
		if(self.visible) {
			//log.log('text', 'text', 'Can\'t show');
			return;
		}

		//log.log('text', 'text', 'Show');
		self.visible = true;

		gsap.to(elements, duration, {y: 0, opacity: 1, delay: delay}, staggerDelay * direction);
	}

	function hide(direction) {
		if(!self.visible) return;
		//log.log('text', 'text', 'Hide');
		self.visible = false;

		gsap.to(elements, duration, {y: y * direction * -1, opacity: 0}, staggerDelay * -1);
	}

	function enable() {
		//log.log('text', 'text', 'Enable');
...
module.exports = Text;
},{"../log":34,"../utils/map":50,"gsap":2}],42:[function(require,module,exports){
/*
	Dependencies
*/
...
function TriggerElement(options) {
	...
	*/
	...
	function calculateOffsets() {
		top = getAbsoluteOffsetTop(el, stop);
		bottom = stop.offsetHeight - el.offsetHeight - top;

		self.top = top;
		self.bottom = bottom;
	}

	function getAbsoluteOffsetTop(_el, _stop, _value){
		if(_el === _stop) return _value;
		var value = _el.offsetTop + (_value || 0);
		return getAbsoluteOffsetTop(_el.offsetParent, _stop, value);
	}
...
module.exports = TriggerElement;
},{"../utils/map":50}],43:[function(require,module,exports){
/*
	Dependencies
*/
...
function Player(opt) {
	...
	*/
	...
	var smallPlayerOnXs = el.dataset.smallOnXs;

	if(smallPlayerOnXs) {
		el.classList.add('small-player-xs')
	}

	var spriteCanvas = el.querySelector('.sprite canvas');
	var images = [
		'/wp-content/themes/hellomind/static/images/sprite-player.png'
	];
	...
	var canvas = arc.querySelector('canvas');
	...
	canvas.style.width = arc.offsetWidth + 'px';
	canvas.style.height = arc.offsetHeight + 'px';
	...
	var xy = width / 2;
	var radius = xy - options.thickness / 2;
	...
	*/
	...
	function draw() {
		if(smallPlayerOnXs) return;

		context.clearRect(0, 0, canvas.width, canvas.height);

		context.beginPath();
		context.arc(xy, xy, xy - 4 - options.thickness, 0, 2 * Math.PI, false);
		...
		// Calculate angle
		var angle = (progress.value - options.min) * angleArc / (options.max - options.min);
		var endAt = startAt + angle;

		context.beginPath();
		context.arc(xy, xy, radius, startAt, endAt, false);
	...
	function onEnded() {
		...
		hasEnded = frame === endFrame ? true : false;
	}

	function setProgress(_progress) {
		gsap.to(progress, 1, {value: _progress, onUpdate: draw});
	}
...
},{"gsap":2,"stupid-sprite":16,"stupid-tick":18}],44:[function(require,module,exports){
/*
	Dependencies
*/

var gsap = require('gsap');
var page = require('./page')({forceLoader: false});
//var tick = require('stupid-tick')();
//var scrollspy = require('stupid-scrollspy').getInstance({tick: tick});
//var scrollspy = require('stupid-scrollspy').getInstance();

var layout = require('./shift/layout');
...
function Shift(options) {
	...
	*/
	...
	[].slice.call(el.querySelectorAll('.layout-shift')).forEach(function(el) {
		layouts.push(layout({el: el}));
	});
...
},{"./page":38,"./shift/layout":45,"gsap":2}],45:[function(require,module,exports){
/*
	Dependencies
*/
...
function Layout(options) {
	...
	*/
	...
	var scrollspy = Scrollspy({tick: Tick({fps: 25})});

	var layout = scrollspy.add(el);
	var progress = el.querySelector('.progress');
	var ratio = 50;

	var elements = [].slice.call(el.querySelectorAll('[data-ratio]'));

	elements.forEach(function(elem) {
		elem.dataset.ratio = Math.round(Math.random() * 50);
	});

	layout.on('progress', function(_el, _direction, _progress) {
		elements.forEach(function(elem) {
			gsap.to(elem, 1, {y: _progress * ratio * elem.dataset.ratio * -1});
		});
	});
...
},{"gsap":2,"stupid-scrollspy":11,"stupid-tick":18}],46:[function(require,module,exports){
/*
	Dependencies
*/
...
function Slider(options) {
	...
	*/
	...
	var slides = [].slice.call(container.querySelectorAll('.slide'));
	var width;
	var height;
	var containerWidth;
	var tween = {x: 0};
	...
	var calculateHeight = options.calculateHeight || false;

    var slider = new iscroll(el, {
    	scrollX: true,
    	scrollY: false,
    	snap: true,
    	probeType: 3,
	...
	*/
	...
	function bindEvents() {
		...
		slider.on('scroll', scroll);
		slider.on('scrollEnd', scrollEnd);
	...
	function draw() {
		...
		slides.forEach(function(_el) {
			_el.style.width = width + 'px';
			if(calculateHeight) _el.style.height = height + 'px';
		});	

		containerWidth = width * slides.length;
	...
	*/
	...
	function scroll() {
		var percentage = this.x * -1 / containerWidth;
		var progress = Math.min(Math.max(percentage, 0), 1);
		event.trigger('scroll', progress);

		tween.x = this.x;
	}

	function scrollEnd() {
		event.trigger('scrollend', slider.currentPage.pageX)
	}

	function scrollTo(_progress) {
		var x = (containerWidth) * _progress * -1;

		tweenScroll(x);		
	}

	function tweenScroll(x, _duration) {
		_duration = typeof _duration === 'undefined' ? duration : _duration;

		gsap.to(tween, _duration, {
			x: x, 
			ease: Expo.easeOut, 
			onUpdate: function() {
				slider.scrollTo(tween.x, 0);	
			}
		})	
	}

	function goToPage(index) {
		var x = slider.pages[index][0].x;


		//var _duration = Math.abs(slider.currentPage.pageX - index) * duration;
		tweenScroll(x, 1);
	}

	function enable() {
		container.style.left = '';
		container.classList.remove('disable-transform');

		setTimeout(function() {
			container.classList.remove('disable-transition');	
		}, 0)
	}
	...
	function disable() {
		...
		container.classList.add('disable-transform', 'disable-transition');
...
},{"gsap":2,"iscroll":1,"stupid-event":8}],47:[function(require,module,exports){
/*
	Dependencies
*/
	...
	function hide(obj) {
		gsap.to(el, 1, {
			...
			onComplete: function() {
				...
				if(typeof obj === 'object' && typeof obj.onComplete === 'function') obj.onComplete();
			}
		})
	}
...
},{"gsap":2}],48:[function(require,module,exports){
/*
	Dependencies
*/
var gsap = require('gsap');
...
function Stagger(options) {
	...
	*/
	...
	var elements = options.elements;
	var duration = options.duration || .5;
	var offset = options.offset || window.innerHeight / -10;
	var ease = options.ease;
	var delay = options.delay || duration / 5;
	...
	*/
	...
	function show() {
		...
		gsap.staggerTo(elements, duration, {
			y: 0,
			opacity: 1,
			ease: ease
		}, delay);
	...
	function reset() {
		gsap.set(elements, {y: offset, opacity: 0});	
	}
...
},{"gsap":2}],49:[function(require,module,exports){
/*
	Dependencies
*/
...
function Thumbnails(options) {
	...
	*/
	...
	var thumbnails = [].slice.call(el.querySelectorAll('.thumbnail'));

	var nextButton = el.querySelector('button.next');
	var prevButton = el.querySelector('button.prev');

	var scrollerContainer = el.querySelector('ul');

	var scrollerWidth = 0
	var offset = el.offsetWidth

	var scroller = new iscroll(el.querySelector('.scroller'), {
  	scrollX: true,
  	scrollY: false,
  	snap: 'li',
  	probeType: 3,
  ...
  scroller.on('scroll', function() {
  	console.log(scroller.currentPage, scrollerWidth - offset)
  	scroller.currentPage.pageX === 0 ? prevButton.classList.add('disabled') : prevButton.classList.remove('disabled')
  	scroller.currentPage.x === (scrollerWidth - offset) * -1 ? nextButton.classList.add('disabled') : nextButton.classList.remove('disabled')
  })
  ...
  nextButton.addEventListener('click', function() {
	...
	thumbnails.forEach(bindEvents);

	/**
	 * Calculate width of scroll contrainer, ugly way, but works
	 */
	calculateWidth()
	...
	*/
	...
	function bindEvents(thumbnail) {
		thumbnail.addEventListener('click', click);
	}


	/** calculate width for scroller */
	function calculateWidth () {
		offset = el.offsetWidth
		scrollerWidth = 0
		thumbnails.forEach(function(thumbnail, index) {
			scrollerWidth += thumbnail.offsetWidth
			if (index < thumbnails.length) scrollerWidth += 20
		})
		scrollerContainer.style.width = scrollerWidth + 'px'
		setTimeout(function() {
			scroller.refresh()
			console.log('new width set: ' + scrollerWidth)
		}, 0)
	}
	...
	*/
	...
	function click() {
		var index = thumbnails.indexOf(this);

		set(index);
		event.trigger('click', index);
	}
...
},{"iscroll":1,"stupid-event":8}],50:[function(require,module,exports){
function map (_value, _istart, _istop, _ostart, _ostop) {
    var ostart = _ostart === undefined ? 0 : _ostart;
    var ostop = _ostop === undefined ? 1 : _ostop;
    if(_ostart == _ostop) return _ostop;
    if(_value < _istart) return ostart;
    if(_value > _istop) return ostop;
...
var gsap = require('gsap');
var Event = require('stupid-event');
var Container = require('./video/container');
var VideoElement = require('./video/videoelement');
var MouseInterval = require('./video/mouseinterval');
var Player = require('./player');
var Poster = require('./video/poster');
...
function Video(options) { 
	...
	*/
	...
	var self = {};
	var elements = {};
	var el = options.el || undefined;
	elements.player = el.querySelector('.partial-player');
	elements.back = el.querySelector('.back');
	...
	var controller;
	var video;
	var timer = MouseInterval({el: el, onActive: onActive, onInactive: onInactive});
	var player = Player({el: elements.player});
	var containerEl = options.containerEl || el;
	var playInline = el.dataset.videoPlayInline || false;

	/*
		No reliable way to check whether a device only supports the native player without playing the video first.
		This ugly check takes care of the iPhone and iPod, but might be an issue on older Androud devices
	 */
	var inlineVideoNotSupported = ((navigator.userAgent.match(/iPhone/i)) || (navigator.userAgent.match(/iPod/i)));

	if(playInline) containerEl.classList.add('play-inline');
	...
	*/
	...
	function init() {
		//log.log('video', 'Video', 'Initalize');
		// Poster
		poster = Poster({el: el.querySelector('.poster')})

		// Container
		container = Container({ el: containerEl, ease: ease, duration: duration});

		// Video element
		video = VideoElement({
			el: el.querySelector('video'), 
			time: {
				self: el.querySelector('.time'),
				progress: el.querySelector('.time .progress'),
				duration: el.querySelector('.time .duration')
			}
		});
	...
	function bindEvents() {
		//el.addEventListener('click', toggle);
		elements.player.addEventListener('click', enable);
		elements.back.addEventListener('click', disable);


		container.on('enabled', onEnabled);
		container.on('disabled', onDisabled);
		container.on('completed', function() {
	...
	function toggle(e) {
		return;
		if(e.target === elements.player) return;
		if(isActive) {
			video.toggle();
			//log.log('video', 'Video', 'toggle video');
		}
	}

	function onEnabled() {
		// When container has zoomed in and transitions have been finished
		video.play();
	}

	function onDisabled() {
		// When container triggers disable event
		video.pause();
	}
	...
	function enable(e) {
		//log.log('video', 'Video', 'click click click');
		if(isActive) {
			video.toggle();
			return;
		}

		event.trigger('beforetransition');
		
		isActive = true;

		

		if(inlineVideoNotSupported) {
			video.play();
			return;
		}
...
},{"./log":34,"./player":43,"./video/container":52,"./video/mouseinterval":53,"./video/poster":54,"./video/videoelement":55,"gsap":2,"stupid-event":8}],52:[function(require,module,exports){
/*
	Dependencies
*/
...
function Container(options) { 
	...
	*/
	...
	function calculateBoundingRect() {
		var _coords = parent.getBoundingClientRect();	

		coords = {
			left: _coords.left,
			top: _coords.top,
			right: window.innerWidth - _coords.right,
			bottom: window.innerHeight - _coords.bottom
		}

		var width = _coords.width / window.innerWidth;
		var height = _coords.height / window.innerHeight;

		duration = 1 - (width + height) / 2;
	}

	function setPosition(_coords) {
		for(var key in _coords) {
			el.style[key] = _coords[key] + 'px';
		}
	}
	...
	function enable() {
		...
		//alert(coords.width / coords.height);

		setPosition(coords);
		/*gsap.to(el, duration / 2, { ease: ease, left: 0, right: 0, onComplete: function() {
			gsap.to(el, duration / 2, {ease: ease, bottom: 0, top: 0, onComplete: function() {
				el.classList.add('completed');
		...
		gsap.to(el, duration, { ease: ease, left: 0, right: 0, bottom: 0, top: 0, onComplete: function() {
			el.classList.add('completed');
			event.trigger('enabled');
		} });
	...
	function disable() {
		...
		gsap.to(el, duration, { ease: ease, left: coords.left, top: coords.top, right: coords.right, bottom: coords.bottom, onComplete: onComplete });		
	}

	function onComplete() {
		el.classList.remove('active');
		el.style.position = '';
		setPosition({left: 0, top: 0, right: 0, bottom: 0});
		body.style.overflow = '';
...
},{"gsap":2,"stupid-event":8}],53:[function(require,module,exports){
/*
	Dependencies
*/

function MouseInterval(options) { 
	...
	*/
	...
	function inactive() {
		...
		// Callback
		if(typeof options.onInactive === 'function') options.onInactive();
	}

	function reset() {
		clear();
	    timeout = setTimeout(inactive, interval);
	}
...
function Poster(options) {
	...
	*/
	...
	if(el) {
		var scale = options.scale || el.dataset.posterScale || 1.75;
		var duration = options.duration || 1;
		var ease = options.ease || Circ.easeOut;

		gsap.set(el, {scale: scale, force3D: true});
	}

	function enable(hideOnComplete) {
		if(!el) return;

		var opacity = hideOnComplete ? 0 : 1;

		gsap.to(el, duration, {scale: 1, force3D: true, opacity: opacity, ease: ease});
	}

	function disable() {
		if(!el) return;
		gsap.to(el, duration, {scale: scale, force3D: true, opacity: 1, ease: ease});	
	}
...
},{"gsap":2}],55:[function(require,module,exports){
/*
	Dependencies
*/
...
function VideoElement(options) { 
	...
	*/
	...
	var self = {};
	var event = Event();
	var video = options.el || undefined;
	...
	*/
	...
	function init() {
		...
		// Only realiable cross-browser way to trigger the loadedmetadata event - ugly tho
		video.play();
		setTimeout(function() {
			video.pause();
		}, 0);
	...
	function play() {
		isPlaying = true;
		video.click();
		//log.log('videoelement', 'videoelment', 'play');
		video.play();
		event.trigger('play');
	}
	...
	function pause() {
		isPlaying = false;
		//log.log('videoelement', 'videoelment', 'pause');
		video.pause();
		event.trigger('pause');
	}

	function toggle() {
		//log.log('videoelement', 'videoelment', 'toggle');
	...
	function onEnded() {
		//log.log('videoelement', 'videoelment', 'onEnded');
		event.trigger('ended');
	}

	function onProgress(e) {
		event.trigger('playing', video.currentTime / video.duration);

		if(time) {
			if(!time.hasLoaded) onDurationChange();
			time.progress.innerHTML = parseTime(video.currentTime);
		}
	}

	function onDurationChange() {
		time.self.classList.add('active');
		time.hasLoaded = true;
		time.duration.innerHTML = parseTime(video.duration);	
	}

	function parseTime(seconds) {
		//var hours = zeroPad(parseInt( seconds / 3600 ) % 24);
		var minutes = zeroPad(Math.floor(parseInt( seconds / 60 )) % 60);
		var seconds = zeroPad(Math.floor(seconds % 60));
